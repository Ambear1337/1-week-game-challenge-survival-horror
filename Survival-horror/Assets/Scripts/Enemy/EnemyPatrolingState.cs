...
public class EnemyPatrolingState : EnemyBaseState
{
    ...
    void Patrol()
    {
        if (!manager._playerInSight)
        {
            ...
            manager.agent.SetDestination(manager.patrolPoints[manager._currentPatrolPoint].position);
        }
        else
        {
    ...
    {
        if (Vector3.Distance(manager.transform.position, manager.patrolPoints[manager._currentPatrolPoint].position) < 2f)
        {
            manager._currentPatrolPoint = (manager._currentPatrolPoint + 1) % manager.patrolPoints.Length;
        }
    }
}
